...
export const getFollowUpEmailTemplate = (
  ...
  const templates: Record<string, { subject: string; html: string }> = {
    INVOICE_SENT: {
      ...
      html: `<p>Hi ${data.clientName},</p><p>Please find attached your invoice <strong>${data.invoiceNumber}</strong> for <strong>${data.currency} ${data.amount}</strong>, due on <strong>${data.dueDate}</strong>.</p>${payButton(data.payLink)}<p>Thank you for your business.</p><p>${senderName(data)}</p>`,
    },
    PRE_DUE_REMINDER: {
      subject: `Reminder: Invoice ${data.invoiceNumber} due soon`,
      html: `<p>Hi ${data.clientName},</p><p>This is a friendly reminder that invoice <strong>${data.invoiceNumber}</strong> for <strong>${data.currency} ${data.amount}</strong> is due on <strong>${data.dueDate}</strong>.</p>${payButton(data.payLink)}<p>${senderName(data)}</p>`,
    },
    FIRST_OVERDUE: {
      subject: `Invoice ${data.invoiceNumber} is now overdue`,
      html: `<p>Hi ${data.clientName},</p><p>Invoice <strong>${data.invoiceNumber}</strong> for <strong>${data.currency} ${data.amount}</strong> was due on <strong>${data.dueDate}</strong> and has not yet been settled. Please arrange payment at your earliest convenience.</p>${payButton(data.payLink)}<p>${senderName(data)}</p>`,
    },
    SECOND_OVERDUE: {
      subject: `Second notice: Invoice ${data.invoiceNumber} overdue`,
      html: `<p>Hi ${data.clientName},</p><p>We have not yet received payment for invoice <strong>${data.invoiceNumber}</strong> totalling <strong>${data.currency} ${data.amount}</strong>, which was due on <strong>${data.dueDate}</strong>. Please make payment as soon as possible.</p>${payButton(data.payLink)}<p>${senderName(data)}</p>`,
    },
    FINAL_NOTICE: {
      subject: `Final notice: Invoice ${data.invoiceNumber}`,
      html: `<p>Hi ${data.clientName},</p><p>This is a final notice regarding invoice <strong>${data.invoiceNumber}</strong> for <strong>${data.currency} ${data.amount}</strong>. This invoice is now significantly overdue. Please contact us immediately to resolve this matter.</p>${payButton(data.payLink)}<p>${senderName(data)}</p>`,
    },
  };
